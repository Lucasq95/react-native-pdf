...
export default class Pdf extends Component {
  ...
  _onChange = event => {
    ...
    if (message.length > 0) {
      if (message[0] === "loadComplete") {
        ...
        this.props.onPageCoords &&
          this.props.onPageCoords(message[1], message[2], message[3]);
      } else if (message[0] === "deviceCoords") {
        this.props.onDeviceCoords &&
          this.props.onDeviceCoords(message[1], message[2], message[3]);
      }
    }
  };
  ...
  mapDeviceCoordsToPage = (x, y, width, height) => {
    ...
    UIManager.dispatchViewManagerCommand(findNodeHandle(this._root), 0, [
      deviceX,
      deviceY,
      containerWidth,
      containerHeight
  ...
  mapPageCoordsToDevice = (x, y, width, height, id) => {
    const pageX = Math.round(x);
    const pageY = Math.round(y);
    const containerWidth = Math.round(width);
    const containerHeight = Math.round(height);

    UIManager.dispatchViewManagerCommand(findNodeHandle(this._root), 1, [
      pageX,
      pageY,
      containerWidth,
      containerHeight,
        ...
        <View style={[this.props.style, { overflow: "hidden" }]}>
          {!this.state.isDownloaded ? (
            ...
            <PdfView
              ...
              onDeviceCoords={this.props.onDeviceCoords}
